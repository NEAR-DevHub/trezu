...
export function convertUrlParamsToApiFilters(
    ...
    if (proposersParam) {
        const proposersData = parseFilterData(proposersParam) as {
            operation: string;
            users: string[];
        } | null;
    ...
    if (approversParam) {
        const approversData = parseFilterData(approversParam) as {
            operation: string;
            users: string[];
        } | null;
    ...
    if (recipientsParam) {
        const recipientsData = parseFilterData(recipientsParam) as {
            operation: string;
            users: string[];
        } | null;
    ...
    if (proposalTypesParam) {
        const proposalTypesData = parseFilterData(proposalTypesParam) as {
            operation: string;
            selected: string[];
        } | null;
        if (
            proposalTypesData?.selected &&
            proposalTypesData.selected.length > 0
        ) {
    ...
    if (tokenParam) {
        ...
        if (tokenData?.token) {
            ...
            if (tokenData.operation === "Is") {
                ...
                if (
                    tokenData.amountOperation &&
                    (tokenData.minAmount || tokenData.maxAmount)
                ) {
                    switch (tokenData.amountOperation) {
                        case "Between":
                            if (tokenData.minAmount)
                                filters.amount_min = tokenData.minAmount;
                            if (tokenData.maxAmount)
                                filters.amount_max = tokenData.maxAmount;
                            break;
                        case "Equal":
                            if (tokenData.minAmount)
                                filters.amount_equal = tokenData.minAmount;
                            break;
                        case "More Than":
                            if (tokenData.minAmount)
                                filters.amount_min = tokenData.minAmount;
                        ...
                        case "Less Than":
                            if (tokenData.minAmount)
                                filters.amount_max = tokenData.minAmount;
                            break;
                    }
                }
    ...
    if (createdDateParam) {
        const dateData = parseFilterData(createdDateParam) as {
            operation: string;
    ...
    if (myVoteParam && userId) {
        const myVoteData = parseFilterData(myVoteParam) as {
            operation: string;
            selected: string[];
        } | null;
