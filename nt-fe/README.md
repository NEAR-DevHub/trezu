...
- `INVITE_ONLY_ENABLED` - `true` gates `/create` behind invite links (`/create?ref=<code>`), adds `X-Robots-Tag: noindex, nofollow`, and requires the same setting on the backend (default: `false`)
- `INVITE_CODES` - server-only, comma-separated, case-sensitive invite codes; must match the backend's list. Never use a `NEXT_PUBLIC_` name for it
