//! TODO(confidential-v2): remove with the dual-write in the cleanup branch.
//!
//! Legacy read adapter serving confidential DAOs from
//! `gold_confidential_history_events` when `UNIFIED_GOLD_LEDGER_READS` is
//! off. The projector dual-writes that table so this path stays live.
